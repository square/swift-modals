...
/// Encapsulates listening for system keyboard updates, plus transforming the visible frame of the
/// keyboard into the coordinates of a requested view.
///
/// You use this class by providing a delegate, which receives callbacks when changes to the
/// keyboard frame occur. You would usually implement the delegate somewhat like this:
///
/// ```
/// func keyboardFrameWillChange(
///    for observer : KeyboardObserver,
...
final class KeyboardObserver {
    ...
    /// The global shared keyboard observer. Why is it a global shared instance? We can only know
    /// the keyboard position via the keyboard frame notifications.
    ///
    /// If a keyboard observing view is created while a keyboard is already on-screen, we'd have no
    /// way to determine the keyboard frame, and thus couldn't provide the correct content insets to
    /// avoid the visible keyboard.
    ///
    /// Thus, the `shared` observer is set up on app startup to avoid this problem.
...
extension ToastPresentationViewController {
    ...
    /// It ensures that all Toasts will correctly avoid the keyboard
